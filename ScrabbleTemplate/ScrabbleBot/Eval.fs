module internal Eval
    ...
    let modu (a : SM<int>) (b : SM<int>) =  a >>= (fun x -> b >>= (fun y -> if y = 0 then fail DivisionByZero else ret(x % y)))
    let charToInt (c: SM<char>) = c >>= (fun x -> ret(int x))   
    ...
    let rec arithEval a : SM<int> = 
        ...
        | CharToInt (c: cExp) -> charToInt (charEval c)    
    ...
    let rec boolEval b : SM<bool> =
        ...
        | IsVowel c -> charEval c >>= (fun x -> ret("aeiouæøåAEIOUÆØÅ" |> Seq.contains x))
